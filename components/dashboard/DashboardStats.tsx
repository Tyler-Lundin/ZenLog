  ...
  return (
    <div>
      ...
      <ul className="bg-white dark:bg-black font-light text-lg dark:text-white p-4 rounded-br-md rounded-bl-md border-l border-b border-r border-black dark:border-white">
        ...
        <li className="flex justify-between"><span>Exercise Total Volume</span> <span>{exercise.totalVolume || 0} <small className="text-sm"> lbs</small></span></li>
        <li className="flex justify-between"><span>Exercise Total Entries</span> <span>{exercise.totalExercises || 0} <small className="text-sm"> {exercise.totalExercises > 1 ? 'entries' : 'entry'}</small> </span></li>
        <hr className="my-2" />
        ...
        <li className="flex justify-between"><span>Sleep Total Entries</span> <span>{sleep.sleepEntries || 0} <small className="text-sm"> {sleep.sleepEntries ? 'entries' : 'entry'}</small></span></li>
        <hr className="my-2" />
        ...
        <li className="flex justify-between"><span>Bodyweight Entries</span> <span>{weight.weightEntries || 0} <small className="text-sm"> {weight.weightEntries > 1 ? 'entries' : 'entry'}</small></span></li>
        <hr className="my-2" />
        <li className="flex justify-between"><span>Todays Mood</span> <span>{mood.mostRecentMood}</span></li>
        <li className="flex justify-between"><span>Mood Entries</span> <span>{mood.moodEntries} <small className="text-sm"> {mood.moodEntries > 1 ? 'entries' : 'entry'}</small></span></li>
      </ul>
    </div>
  )
}
